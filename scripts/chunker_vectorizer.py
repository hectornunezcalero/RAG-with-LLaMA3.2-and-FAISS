...
def chunker(text, chunk_len=500, overlap=50):
    palabras = text.split()
    chunks = []
    for i in range(0, len(palabras), chunk_len - overlap): # los chunks abarcan desde la primera hasta la última palabra, agrupando los chunks con solapamiento
        chunk = " ".join(palabras[i:i + chunk_len])  # une cada palabras en su/sus chunks correspondiente/es
        chunks.append(chunk)
    return chunks

# vectorización de los textos extraídos
def vectorizer(dir_textos="./txtdata", output="./vector_db", chunk_len=500, overlap=50):
