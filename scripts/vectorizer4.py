...
# limpieza del texto para evitar espacios raros, saltos de línea dobles, etc.
def limpiar_texto(texto):
    texto = re.sub(r'\s+', ' ', texto)  # reemplaza múltiples espacios/saltos/tabs por uno solo
    texto = texto.replace('\u200b', '')  # elimina caracteres invisibles unicode
    return texto.strip()
...
def vectorizer(dir_textos="./txtdata", output="./vector_db", chunk_len=230, overlap=50):
    ...
    for archivo in os.listdir(dir_textos):
        if archivo.endswith(".txt"):
            with open(os.path.join(dir_textos, archivo), "r", encoding="utf-8") as f:
                contenido = f.read()
                contenido = limpiar_texto(contenido)  # limpieza del texto antes del chunkeo
