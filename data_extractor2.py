...
def extract_txt(pdf_path):
    ...
    for page in doc:
        ...
        if 0.3 < left_percent < 0.7:
            # se fusionan izquierda y derecha y se ordenan por posición vertical (y horizontal en caso de empate)
            combined = left + right
            combined.sort(key=lambda b: (b["bbox"][1], b["bbox"][0]))
            final_text += block_process(combined)
        else:
            # se ordena en una sola columna (vertical)
