...
class Llama3GUI:
    ...
    def send_request(self):
        ...
        try:
            server_response = self.client(content)
            if 'content' in server_response:
                response = server_response['content']
            else:
                response = server_response[0]
        except Exception as ex:
            raise ResponseError(f"Error: {ex} for response {server_response} with content: '{content}'")

        # Display the response in the output text area
        self.output_text.delete('1.0', tk.END)
        self.output_text.insert(tk.END, str(response))
...
llama_client = Llama3CLI(api_key="<MASTERKEY>", server_ip="192.168.77.135")
